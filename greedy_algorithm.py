...
############
def min(lst,tour):#finds minimum distance from current city to unvisited city
    m=[0,0]
    for i in range(len(lst)):#i is city,m[0] stores city,m[1] stores distance
        if (i in tour)==True:
            continue
        if m[1]>lst[i] or m[1]==0:
            m[1]=lst[i]
            m[0]=i
    return m
tour=[]
...
while(len(tour)!=len(dist_matrix)):
    tour.append(counter)
    m=min(dist_matrix[counter],tour)
    tour_length+=m[1]
    counter=m[0]
tour_length+=dist_matrix[tour[num_cities - 1]][tour[0]]
tour_length=0
