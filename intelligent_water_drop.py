...
"""Go to line 404 to start, the IWD algorithm and formula's used are based on and derived from this article:
...
def min_dist(current_city,available_cities,mini):
    ...
    # used in the g_soil() function to deal with negative soil values
...
def drops(visited,unvisited,a_tour_length,pos):
    ...
    for i in range(0,num_cities-1):
        ...
        vel_iwd[pos]+=1/(0.01+((1)*(soil_mat[visited[i]][next_city]**2)))
        # formula used: vel_iwd = a_v/(b_v+(c_v*(soil(i,j)**2))
        # a_v=1000000,b_v=0.01,c_v=1

        time=dist_matrix[visited[i]][next_city]/(vel_iwd[pos])
        # formula used: time(i,j) = HUD(i,j)/vel_iwd, HUD is Heuristic Undesirability
        # HUD(i,j)= dist_between(i,j)

        delta_soil=1/(0.01+((1)*time**2))
        # formula used: delta_soil(i,j)= a_s/(b_s+(c_s*(time(i,j)**2))
        # a_s=1,b_s=0.01,c_s=1

        soil_mat[visited[i]][next_city]=(1-0.9)*(soil_mat[visited[i]][next_city])-0.9*delta_soil
    ...
    vel_iwd[pos]+=1/(0.01+((1)*soil_mat[next_city][visited[0]]**2))

    time=dist_matrix[next_city][visited[0]]/(vel_iwd[pos])
    delta_soil=1/(0.01+((1)*time**2))

    soil_mat[next_city][visited[0]]=(1-0.9)*(soil_mat[next_city][visited[0]])-0.9*delta_soil
    ...
    return visited,a_tour_length,soil_iwd[pos]
...
def cycles(num_cycles):
    ...
    for i in range(0,num_cycles):
        ...
        for j in range(0,num_cities):
            ...
            # formula used: soil(i,j)= (1+p_iwd)*soil(i,j)-((p_iwd)*(1/num_cities-1)*soil_iwd)
            # p_iwd = 0.9
            if j==(num_cities-1):
                soil_mat[Best_tour[0][j]][Best_tour[0][0]]=((1+0.9)*soil_mat[Best_tour[0][j]][Best_tour[0][0]])-((0.9)*(1/(num_cities-1))*(Best_tour[2]))
            
            else:
                soil_mat[Best_tour[0][j]][Best_tour[0][j+1]]=((1+0.9)*soil_mat[Best_tour[0][j]][Best_tour[0][j+1]])-((0.9)*(1/(num_cities-1))*(Best_tour[2]))

    print(Best_tour)            
    return Best_tour # Best tour is returned


soil_mat=gen_matrix(10000,[]) # soil_mat is initialized with int_soil = 10000
num_cycles=40 # 40 cycles

Best_tour=cycles(num_cycles) # Best_tour is stored
