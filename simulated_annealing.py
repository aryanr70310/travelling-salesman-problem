...
############
def alternate(pos_swap,adjacent_tour,adjacent_tour_len):
    if pos_swap==(num_cities-1):
        adjacent_tour[0],adjacent_tour[num_cities-1]=adjacent_tour[num_cities-1],adjacent_tour[0]
        adjacent_tour_len=adjacent_tour_len+(dist_matrix[current_tour[num_cities-1]][current_tour[1]]-dist_matrix[current_tour[0]][current_tour[1]])+(dist_matrix[current_tour[0]][current_tour[num_cities-2]]-dist_matrix[current_tour[num_cities-1]][current_tour[num_cities-2]])
    elif pos_swap==(num_cities-2):
        adjacent_tour[pos_swap],adjacent_tour[pos_swap+1]=adjacent_tour[pos_swap+1],adjacent_tour[pos_swap]
        adjacent_tour_len=adjacent_tour_len+(dist_matrix[current_tour[pos_swap]][current_tour[0]]-dist_matrix[current_tour[pos_swap+1]][current_tour[0]])+(dist_matrix[current_tour[pos_swap+1]][current_tour[pos_swap-1]]-dist_matrix[current_tour[pos_swap]][current_tour[pos_swap-1]])
    else:
        adjacent_tour[pos_swap],adjacent_tour[pos_swap+1]=adjacent_tour[pos_swap+1],adjacent_tour[pos_swap]
        adjacent_tour_len=adjacent_tour_len+(dist_matrix[current_tour[pos_swap]][current_tour[pos_swap+2]]-dist_matrix[current_tour[pos_swap+1]][current_tour[pos_swap+2]])+(dist_matrix[current_tour[pos_swap+1]][current_tour[pos_swap-1]]-dist_matrix[current_tour[pos_swap]][current_tour[pos_swap-1]])
    return adjacent_tour,adjacent_tour_len
T=1
current_tour=[]
current_tour_len=0
for i in range(len(dist_matrix)):
    ...
    if i==len(dist_matrix)-1:
        current_tour_len+=dist_matrix[i][0]
        break
    current_tour_len+=dist_matrix[i][i+1]
min_tour=[]
min_tour.append([])
min_tour[0].extend(current_tour)
min_tour.append(current_tour_len)
counter=1
...
while T>0:
    ...
    adj=[]
    adj.append([])
    adj[0].extend(current_tour)
    adj.append(current_tour_len)
    p=random.randint(0,num_cities-1)
    adj=alternate(p,adj[0],current_tour_len)
    if adj[1]>current_tour_len:
        if random.random()<=(e**((adj[1]-current_tour_len)/T)):#fix maths
            current_tour=[]
            current_tour.extend(adj[0])
            current_tour_len=adj[1]
            continue
        continue
    if adj[1]<current_tour_len:
        current_tour=[]
        current_tour.extend(adj[0])
        current_tour_len=adj[1]
        if current_tour_len<min_tour[1]:
            min_tour[0]=[]
            min_tour[0].extend(current_tour)
            min_tour[1]=current_tour_len
    counter+=1
    T-=1/counter
tour=[10, 6, 42, 24, 19, 9, 3, 7, 14, 32, 43, 27, 41, 37, 33, 35, 17, 23, 21, 29, 47, 39, 15, 45, 2, 25, 1, 4, 26, 22, 44, 40, 5, 38, 13, 31, 12, 8, 36, 11, 28, 0, 30, 46, 34, 18, 16, 20]
tour_length=min_tour[1]
