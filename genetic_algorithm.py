...
############
def generate_sample_routes(a_tour,population):
    for i in range(0,num_cities):
        a_tour.append(i)
...
def fitness_calculator(population,fitness):
    for i in range(0,pop_size):
        ...
        fitness[i]=invert_distance(fitness[i])
    return fitness
...
def selecting_next_gen(population,new_population,mutation_rate):
    parent_sample=int(0.8*pop_size)
    new_population.extend(create_children(population[:parent_sample],[],parent_sample))#children
    for i in range(len(new_population)):
        if random.random()<mutation_rate:
            new_population[i]=mutate(new_population[i],0,0)#Children may mutate
    for i in range(0,(int)(0.2*pop_size)):
        new_population.append(population[i])#Elites
    return new_population

def genetics(current_population,population_fitness,pop_size):
    for i in range(pop_size):
        ...
        current_population=selecting_next_gen(current_population,[],0.1)
        population_fitness=fitness_calculator(current_population,[0]*pop_size)
    fitness_sorted=sort(population_fitness,current_population,[0,[]])
    current_population,population_fitness=fitness_sorted[0],fitness_sorted[1]
    print(1/(population_fitness[0]))
    return

pop_size=200
current_population=generate_sample_routes([],[])
population_fitness=fitness_calculator(current_population,[0]*pop_size)
genetics(current_population,population_fitness,pop_size)
# for i in range(100):
#     fitness_sorted=sort(population_fitness,current_population,[0,[]])
#     current_population,population_fitness=fitness_sorted[0],fitness_sorted[1]
#     current_population=selecting_next_gen(current_population,[],0.1)
...
# current_population,population_fitness=fitness_sorted[0],fitness_sorted[1]
# print(1/(population_fitness[0]))
tour_length=int(1/(population_fitness[0]))
tour=current_population[0]
tour_length=0
