...
"""Go to line 362 to start, the ACO algorithm and formula's used are based on and derived from this video:
...
def adjust_pheromones(m1,m2):
    # This function is used in updating the pheromone counts after a group of ants complete a tour
...
def prob_calc(current_city,available_cities,prob_list):
    ...
    for i in range(0,len(available_cities)):
        ...
        # a=1.0, B=6.0, 0.01 is a small amount to prevent division by 0 error
...
start=random.randint(0,num_cities-1) # home of the ants is randomly selected
phero_matrix=(gen_matrix(1,[])) # phero_matrix is intitalized to a matrix full of 1s

Tours=excursions(100,[]) # mintour is returned after num_excursions=50 excursions/iterations take place
tour,tour_length=Tours[0],Tours[1] # values of mintour are stored
